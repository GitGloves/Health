...
mydata<-read.csv ("C:\\Users\\rcase\\Downloads\\nc.csv", header=TRUE)
...
split <- sample(seq_len(nrow(mydata)), size = 0.7 * nrow(mydata))
...
-   Print the value of **`MSPE_linear`** to the console using the **`print()`** function.
    ...
    # fit linear regression model on train_data
    linearmodel <- lm(weight ~ ., data = train_data)

    # predict weight using linearmodel
...
```{r,echo=TRUE}
# calculate mean squared prediction error for linear model
MSPE_linear <- mean((test_data$weight - predict(linearmodel, newdata = test_data))^2)

# calculate mean squared prediction error for GAM model
MSPE_gam <- mean((test_data$weight - predict(gam_model, newdata = test_data, type = "response"))^2)
...
The mean squared prediction error (MSPE) obtained from the linear regression model (MSPE_linear = 1.032627) in Task 2 is higher than the MSPE obtained from the generalized additive model (MSPE_gam = 0.9667269) in the previous task. This indicates that the GAM model performs better than the linear regression model in predicting the 'weight' variable in the test_data dataset.
